...
function start() {
  ...
  function informIfOutdated() {
    ...
    if ('@kinvolk/headlamp-plugin' in outdated) {
      // const latest = outdated['@kinvolk/headlamp-plugin'].latest;
      // const url = `https://github.com/kinvolk/headlamp/releases/tag/headlamp-plugin-${latest}`;
...
function upgrade(packageFolder, skipPackageUpdates) {
  ...
  function upgradeHeadlampPlugin(skipPackageUpdates) {
    if (skipPackageUpdates === true) {
      console.log('Skipping package updates.');
      return true;
    }
  ...
  function upgradePackage(folder, skipPackageUpdates) {
    ...
    if (!upgradeHeadlampPlugin(skipPackageUpdates)) {
      return false;
    }
